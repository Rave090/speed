...
if (empty($date)) {
    header("Location: edit.php?id=$editId&error=Date cannot be empty.");
    exit();
}
// Function to calculate payment difference
function calculatePaymentDifference($startTime, $endTime) {
    $startMinutes = strtotime($startTime) / 60;
    $endMinutes = strtotime($endTime) / 60;
    $timeGap = $endMinutes - $startMinutes;
    $payment = $timeGap * 0.167; // Assuming the rate is Rs. 0.167 per minute
    return $payment;
}
...
if ($editResult->num_rows == 1) {
    $editData = $editResult->fetch_assoc();
     // Calculate the payment difference after incrementing end time
     $incrementedEndTime = date('H:i', strtotime($editData['end_time']) + 3600); // Increment by 1 hour
     $actualPayment = calculatePaymentDifference($editData['start_time'], $editData['end_time']);
     $incrementedPayment = calculatePaymentDifference($editData['start_time'], $incrementedEndTime);
     $paymentDifference = abs($actualPayment - $incrementedPayment);
} else {
...
<p>your payment: Rs. <?php echo $paymentDifference; ?></p>
